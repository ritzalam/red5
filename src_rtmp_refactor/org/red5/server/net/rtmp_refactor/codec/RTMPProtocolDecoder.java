...
import org.red5.server.net.rtmp_refactor.message.Constants;
import org.red5.server.net.rtmp_refactor.message.Header;
import org.red5.server.net.rtmp_refactor.message.Packet;
import org.red5.server.net.rtmp_refactor.message.SharedObjectTypeMapping;
...
public class RTMPProtocolDecoder implements Constants, SimpleProtocolDecoder, IEventDecoder {
	...
	public ISharedObjectMessage decodeSharedObject(ByteBuffer in) {
		...
		final SharedObjectMessage so = new SharedObjectMessage();
		
		final Input input = new Input(in);
		so.setName(Input.getString(in));
		// Read version of SO to modify
		so.setVersion(in.getInt()); 
		// Read persistence informations
		so.setPersistent(in.getInt()==2);
		// Skip unknown bytes
		in.skip(4); 
		// Parse request body
